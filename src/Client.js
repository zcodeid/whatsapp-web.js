...
class Client extends EventEmitter {
    ...
    async initialize() {
        ...
        if (
            ...
            this.options.puppeteer.browserWSEndpoint
        ) {
            browser = await chromium.puppeteer.connect(this.options.puppeteer);
            page = await browser.newPage();
        } else {
            browser = await chromium.puppeteer.launch(this.options.puppeteer);
        ...
        if (isMD) {
            throw new Error(
                'Multi-device is not yet supported by whatsapp-web.js. Please check out https://github.com/pedroslopez/whatsapp-web.js/pull/889 to follow the progress.'
            );
        }
